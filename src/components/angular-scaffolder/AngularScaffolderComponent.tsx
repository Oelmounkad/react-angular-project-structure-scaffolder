...
const AngularScaffolderComponent = () => {
  const {
    isOpen: isOpenAddModule,
    onOpen: onOpenAddModule,
    onClose: onCloseAddModule,
  } = useDisclosure();
  const {
    isOpen: isOpenAddGlobalService,
    onOpen: onOpenAddGlobalService,
    onClose: onCloseAddGlobalService,
  } = useDisclosure();
  ...
  const saveAddGlobalServiceModal = () => {
    ...
    const newGlobalService: IService = {
      ...
      name: `${globalServiceName
        .charAt(0)
        .toUpperCase()}${globalServiceName.slice(1)}Service`,
    };
  ...
  const generateAngularProject = () => {
    ...
    axios({
      url: "https://angular-react-scaffolder-node-api.onrender.com/angular-project-scaffolder",
      method: "POST",
  ...
  return (
    <div className="angular-scaffolder-wrapper">
      ...
      <Stack direction="row" spacing={4} align="center" justifyContent="center">
        ...
        <Button
          onClick={onOpenAddGlobalService}
          leftIcon={<AddIcon />}
          colorScheme="yellow"
        >
          Add Global Service
      ...
      <div
        className={
          modules.length > 0
            ? "modules-services-wrapper-gapped"
            : "modules-services-wrapper"
        }
      >
